...
install `nox` and `taskipy`, install the required dependencies in your environment
(make sure you have a virtualenv activated if you don't want it to install them globally!)
and set up [`pre-commit`](https://pre-commit.com/#quick-start) hooks (see further below):
```
pip install nox taskipy
task setup_env
```

There are a few tasks that lint/test the code, enforcing certain quality standards:
...
A PR cannot be merged as long as there are failed tasks.
