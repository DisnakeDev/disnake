...
class CPythonmacOsFramework(CPython):
    ...
    def can_describe(cls, interpreter):
        return is_mac_os_framework(interpreter) and super(CPythonmacOsFramework, cls).can_describe(
            interpreter
        )
...
def fix_mach_o(exe, current, new, max_size):
    ...
    try:
        logging.debug(
            "change Mach-O for %s from %s to %s", ensure_text(exe), current, ensure_text(new)
        )
