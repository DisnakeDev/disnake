...
class PipInstall(object):
    ...
    def _records_text(self, files):
        record_data = "\n".join(
            "{},,".format(os.path.relpath(ensure_text(str(rec)), ensure_text(str(self._image_dir))))
            for rec in files
        )
        return record_data
    ...
    def _generate_new_files(self):
        ...
        try:
            ...
            rel = os.path.relpath(
                ensure_text(str(self._creator.script_dir)), ensure_text(str(self._creator.purelib))
            )
    ...
    def _uninstall_dist(dist):
        ...
        for name in (
            (i.split(",")[0] for i in record.read_text().splitlines()) if record.exists() else ()
        ):
            path = dist_base / name
            if not any(
