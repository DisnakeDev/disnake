...
def test_find() -> None:
    pred = lambda i: i == 42  # pyright: ignore[reportUnknownLambdaType] # type: ignore
    assert utils.find(pred, []) is None
    assert utils.find(pred, [42]) == 42
    assert utils.find(pred, [1, 2, 42, 3, 4]) == 42

    pred = lambda i: i.id == 42  # pyright: ignore[reportUnknownLambdaType] # type: ignore
...
def test_maybe_cast() -> None:
    convert = lambda v: v + 1  # pyright: ignore[reportUnknownLambdaType] # type: ignore
    default = object()
